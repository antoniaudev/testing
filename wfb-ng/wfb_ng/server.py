...
def init_wlans(max_bw, wlans):
    ...
    try:
        ...
        for wlan in wlans:
            ...
            if isinstance(settings.common.wifi_freqs, dict):
                freq = settings.common.wifi_freqs.get(wlan)
                if freq:
                    yield call_and_check_rc('iw', 'dev', wlan, 'set', 'freq', str(freq))
                else:
                    freq = settings.common.wifi_freqs
                    yield call_and_check_rc('iw', 'dev', wlan, 'set', 'freq', str(freq))
